...
- **Resize, rearrange, and export the layout** — drag any panel by its header to reposition it, drag its bottom-right corner to resize it (capped at a maximum size, and double-click the handle to reset a panel straight back to its default size). If a panel's own resize handle ever ends up out of reach (e.g. a very tall panel in a short browser window), click the **↺** button in its header instead — it resets the panel to its default size from anywhere, no hunting for the corner required. Click **Auto-arrange** to snap everything back to a clean tree layout (respecting each panel's current size), and **Export layout as PNG** to save the whole assembled panel-and-connector diagram as one image.
- **Resize the statistics panel** — drag the thin divider between the panel workspace and the **Statistics** table (below it) up or down to trade vertical space between the two; double-click the divider to reset it to its default split. Handy for reclaiming workspace height on a smaller screen.
...
- **Save/open a project, like an RStudio workspace** — **Save Project…** in the sidebar writes every loaded sample's *raw event data* plus its gates, panels, colors, axis labels, and stats-annotation positions, and the whole Layout collage, to a single `.fcsproj` file (you're prompted to name it). **Open Project…** restores all of it exactly — you don't need the original `.fcs` files again to pick up where you left off.
- **Recent projects** — every **Save Project…**/**Open Project…** also keeps a local copy in the browser (IndexedDB, capped at the 6 most recent), listed under **Recent projects** in the sidebar. Click one to reopen it instantly — no file picker, and no need for the downloaded `.fcsproj` file to still be on disk — even after closing the tab or restarting the browser. Saving again under the same name updates that entry in place rather than piling up duplicates; **×** removes a single entry, **Clear** removes them all (this never touches your downloaded files, only the local copy).
...
18. Click **Save Project…** at the top of the sidebar any time to name and download a `.fcsproj` file with everything currently loaded — samples' raw data, gates, panels, colors, custom axis labels, the Layout collage, and the Notebook. Later, click **Open Project…** and pick that file to restore the exact same workspace (confirms first if you already have samples loaded, since it replaces them). Every save/open also appears under **Recent projects** right below — click one there to reopen it straight away, even in a brand-new browser session, without hunting for the downloaded file.
...
│   └── recentProjects.ts   IndexedDB-backed "Recent projects" list: keeps a local copy of each save/open for instant reopening
├── integrations/
│   └── dropbox.ts          Dropbox Chooser: script loading + a Promise wrapper around the popup file picker
├── components/
│   ├── ProjectControls.tsx Save/Open Project buttons + the Recent projects list (the whole workspace, à la an RStudio .RData workspace)
...
- **Recent projects** are stored in the browser's own IndexedDB, scoped to this site's origin — clearing site data/cookies, using a different browser or profile, or opening in a private/incognito window all mean the list won't be there. It's a convenience cache, not a substitute for actually keeping your downloaded `.fcsproj` files; capped at the 6 most recent, oldest evicted automatically.
