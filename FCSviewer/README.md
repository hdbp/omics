...
6. Drag a panel by its header to move it, or drag its bottom-right corner to resize it; click **Auto-arrange** to lay everything out cleanly again (column widths and row heights adapt to each panel's current size); click **Export layout as PNG** to save the whole diagram as an image.
7. Double-click a panel's title to rename its population, or double-click a gate's name in the **Gating hierarchy** tree — both edit the same underlying name. Click the color swatch in a panel's header (or next to a gate in the tree) to assign that population a color; click the small **×** next to a colored swatch to reset it to the default look.
8. You can also open/focus a gate's panel from the **Gating hierarchy** tree in the sidebar (creating one if it doesn't have a panel open yet).
9. Once you've built a gating hierarchy on one sample, click **Apply to other samples…** in the Gating hierarchy panel to copy the whole strategy — gates *and* the panel layout that views them — onto other loaded samples (this replaces their existing gates and panels).
10. In the **Statistics** panel at the bottom, pick which two parameters to show medians for, then **Export stats CSV** for the whole table (every gate, including each quadrant) or the ⭳ button on any row to export that population's raw events as CSV.
...
│   ├── panelLayout.ts    Panel sizing constants + size-aware auto-layout (tree) algorithm
│   └── types.ts          Sample/Panel data model (Panel carries its own width/height)
├── components/
...
│   ├── GatePanel.tsx       A single plot panel: axes, log toggle, gate drawing/coloring, inline rename, drill-down-to-new-panel
