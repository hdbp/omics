# file: R/proseq_metas.R
#' Internal imports for PRO-seq metaplots
#'
#' @keywords internal
...
#' @param library One of `c("paired","single")`.
#' @param read_endpoint One of `c("full","three_prime")`.
#' @param smoothing_span Optional (0,1] loess span.
#' @return List with long data, summary, and three ggplots.
#' @export
proseq_meta <- function(metadata, feature_center_gr, window = 2000, nf = NULL,
  ...
  for (i in seq_len(nrow(metadata))) {
    ...
    GenomeInfoDb::seqlengths(gr) <- GenomeInfoDb::seqlengths(feature_center_gr) # keep source consistent
    ...
    sense_plus <- IRanges::Views(cov_plus, BiocGenerics::split(fwd, GenomicRanges::seqnames(fwd)))
    antisense_plus <- IRanges::Views(cov_minus, BiocGenerics::split(fwd, GenomicRanges::seqnames(fwd)))
    sense_minus <- IRanges::Views(cov_minus, BiocGenerics::split(rev, GenomicRanges::seqnames(rev)))
    antisense_minus <- IRanges::Views(cov_plus, BiocGenerics::split(rev, GenomicRanges::seqnames(rev)))
...
proseq_3p_meta <- function(metadata, feature_center_gr, window = 2000, smoothing_span = NULL) {
  ...
  for (i in seq_len(nrow(metadata))) {
    ...
    GenomeInfoDb::seqlengths(gr) <- GenomeInfoDb::seqlengths(feature_center_gr) # consistent source
    gr <- GenomicRanges::trim(gr)
