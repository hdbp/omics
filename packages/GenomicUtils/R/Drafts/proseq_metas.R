...
#' @importFrom GenomicRanges GRanges resize seqlengths trim coverage strand Views split seqnames end start
#' @export
proseq_meta <- function(metadata, feature_center_gr, window = 2000, nf = NULL,
  ...
  for (i in seq_len(nrow(metadata))) {
    ...
    GenomicRanges::seqlengths(gr) <- GenomicRanges::seqlengths(feature_center_gr)
    ...
    sense_plus <- GenomicRanges::Views(cov_plus, GenomicRanges::split(fwd, GenomicRanges::seqnames(fwd)))
    antisense_plus <- GenomicRanges::Views(cov_minus, GenomicRanges::split(fwd, GenomicRanges::seqnames(fwd)))
    sense_minus <- GenomicRanges::Views(cov_minus, GenomicRanges::split(rev, GenomicRanges::seqnames(rev)))
    antisense_minus <- GenomicRanges::Views(cov_plus, GenomicRanges::split(rev, GenomicRanges::seqnames(rev)))
